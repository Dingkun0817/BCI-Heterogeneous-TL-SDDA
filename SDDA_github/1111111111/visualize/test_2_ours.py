...
def table1_exp():
    ...
    plt.title('Comparison with Various Sophisticated Approaches with Subject EA')
    plt.xticks([r + 3 * bar_width for r in range(len(datasets))], datasets)

    plt.ylim(60, 78)    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    plt.yticks(np.arange(60, 78, step=5))
    plt.legend()
...
def table2_exp():
    ...
    data = pd.read_csv('exp_session_EA.csv')
    ...
    plt.xlabel('Approaches')
    plt.ylabel('Percentage')
    plt.title('Comparison with Various Sophisticated Approaches with Session EA')
    plt.xticks([r + 3 * bar_width for r in range(len(datasets))], datasets)
    ...
    plt.axhline(y=ISFDA[1]+0.01, color='red', linestyle='dashed', xmin=0.28, xmax=0.48)
    plt.axhline(y=Ours[2]+0.01, color='red', linestyle='dashed', xmin=0.52, xmax=0.716)
    plt.axhline(y=MCC[3]+0.01, color='red', linestyle='dashed', xmin=0.763, xmax=0.957)
    # plt.axhline(y=rf_acc[4]+0.01, color='red', linestyle='dashed', xmin=0.8, xmax=0.953)
