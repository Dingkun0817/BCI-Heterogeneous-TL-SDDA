'''
MCC + DAN
Loss = classify + dan(最后一层MultipleKernelMaximumMeanDiscrepancy) + mcc
'''
...
def train_target(args):
    # X_src, y_src, X_tar, y_tar = read_mi_combine_tar_ldk_avg(args)  # 5个点取平均
    # X_src, y_src, X_tar, y_tar = read_mi_combine_tar_ldk_pearson_avg(args)  # 5个点pearsonr加权
    ...
    # # 创建新数组存储扩增后的数据和标签
    # X_augmented = np.empty((X_src.shape[0] * 4, X_src.shape[1], 250))
    # y_augmented = np.empty(y_src.shape[0] * 4, dtype=y_src.dtype)
    #
    # # 对每个样本进行数据扩增
    ...
    #         start = j * 250
    #         end = start + 250
    #         X_augmented[i * 4 + j] = X_src[i, :, start:end]
    #         y_augmented[i * 4 + j] = y_src[i]
    #
    # # 输出扩增后的数据和标签的形状
    # print("扩增后的数据形状：", X_augmented.shape)
    # print("扩增后的标签形状：", y_augmented.shape)
    #
    #
    #
    # X_src = X_augmented
    ...
    # '''
    # # 创建新数组存储拼接后的数据
    # X_concatenated = np.empty((X_src.shape[0], X_src.shape[1], 1126))
    #
    # # 对每个样本的每个通道进行数据拼接
    # for i in range(X_src.shape[0]):
    #     for j in range(X_src.shape[1]):
    #         X_concatenated[i, j] = np.concatenate((X_src[i, j], X_src[i, j, :125]))
    #
    # # 输出拼接后的数据的形状
    # print("拼接后的数据形状：", X_concatenated.shape)
    # X_src = X_concatenated
    #
    #
    #
    # '''
    # tar拼接
    # '''
    # X_concatenated2 = np.empty((X_tar.shape[0], X_tar.shape[1], 1376))
    #
    # # 对每个样本的每个通道进行数据拼接
    # for i in range(X_tar.shape[0]):
    #     for j in range(X_tar.shape[1]):
    #         X_concatenated2[i, j] = np.concatenate((X_tar[i, j], X_tar[i, j, :250]))
    #
    # # 输出拼接后的数据的形状
    # print("拼接后的数据形状：", X_concatenated2.shape)
    # X_tar = X_concatenated2
...
if __name__ == '__main__':
    ...
    args = argparse.Namespace(N1=N1, data_name1=dataset1,
                              ...
                              sample_rate=250, feature_deep_dim=248)  # 248
