...
def train_target(args):
    ...
    print(f"pos_indices count: {len(pos_indices)}, neg_indices count: {len(neg_indices)}")
    ...
    # alignment_weight = nn.Parameter(torch.tensor(args.alignment_weight, requires_grad=True))
    ...
    while iter_num < max_iter:
        ...
        # print(labels_source)   Imbalanced
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            log_str = 'Task: {}, Iter:{}/{}; Auc = {:.2f}% '.\
                format(args.task_str, int(iter_num // len(dset_loaders["source"])), int(max_iter // len(dset_loaders["source"])), auc_t_te)
