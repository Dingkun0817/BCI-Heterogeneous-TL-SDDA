...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        '''
        classifier_loss = criterion(domain_output_s.narrow(0, 0, labels_source.size(0)), labels_source)

        args.srcweight = 3

        target_adv_1 = domain_output_s.max(1)[1]
        target_adv_2 = domain_output_t.max(1)[1]
        target_adv_src = target_adv_1.narrow(0, 0, labels_source.size(0))
        target_adv_tgt = target_adv_2.narrow(0, 0, labels_target.size(0))

        classifier_loss_adv_src = criterion(domain_output_s.narrow(0, 0, labels_source.size(0)), target_adv_src)

        logloss_tgt = torch.log(1 - F.softmax(domain_output_t.narrow(0, 0, labels_target.size(0)), dim=1))
        classifier_loss_adv_tgt = F.nll_loss(logloss_tgt, target_adv_tgt)

        # transfer_loss = args.srcweight * classifier_loss_adv_src + classifier_loss_adv_tgt
        transfer_loss = args.srcweight * classifier_loss_adv_src
