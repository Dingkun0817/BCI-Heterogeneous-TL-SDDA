...
def train_target(args):
    ...
    # 将3维数组重塑为2维数组
    samples, channels, timepoints = train_x.shape
    train_x_reshaped = train_x.reshape(samples, channels * timepoints)

    # 使用SMOTE进行上采样
    smote = SMOTE(random_state=42)
    train_x_resampled, train_y_resampled = smote.fit_resample(train_x_reshaped, train_y)
    print('train_x_resampled:', train_x_resampled.shape)
    print('train_y_resampled:', train_y_resampled.shape)
    print("+_++_++_++_++__+_+-===========================")

    # 将重采样后的数据重塑回原始形状，以便CSP处理
    train_x_resampled = train_x_resampled.reshape(-1, channels, timepoints)

    print('Training/Test split before CSP:', train_x_resampled.shape, test_x.shape)
    csp = CSP(n_components=5)
    train_x_csp = csp.fit_transform(train_x_resampled, train_y_resampled)
    test_x_csp = csp.transform(test_x)
    print('Training/Test split after CSP:', train_x_csp.shape, test_x_csp.shape)
    approach = 'LDA'
    pred = ml_classifier(approach, False, train_x_csp, train_y_resampled, test_x_csp, return_model=True)
    auc_t_te = np.round(roc_auc_score(test_y, pred), 5)
