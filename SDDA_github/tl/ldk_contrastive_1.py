'''
acc = 72.53，Two stages
第一阶段，将源域的22通道塞进教师EEGNet (截取1001采样点)，由于输出的维度好像是4960，所以再拼一个全连接到248，然后是分类器，分类Loss训练后定死参数
ps: 不做22源域和3源域MMD的话，可以EEGNet后不拼全连接试试
第二阶段，源域和目标域都是（3，1001）塞进stu EEGNet，特征也是128维度，
total_loss = classifier_loss + alignment_loss2(源域和目标域的对齐) + ditillation_loss + transfer_loss(MCC)
'''
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        # if(iter_num == 0):
        #     print("_+__+_+_+_+_+__+_++_+_+_+++_+_++_")
    ...
    保存教师网络的feature
    '''
    teachers_feature = features_source.detach()    # 248 dim
    ...
    while iter_num < max_iter:
        ...
        def distillation(y, labels, teacher_scores, temp, alpha):
            return nn.KLDivLoss()(F.log_softmax(y / temp, dim=1), F.softmax(teacher_scores / temp, dim=1)) * (
                    temp * temp * 2.0 * alpha)   # classifier_loss包含了F.cross_entropy(y, labels) * (1. - alpha)的含义

        ditillation_loss = distillation(outputs_source, labels_source, outputs_source_teacher, temp=5.0, alpha=0.7)   # 5  0.7
        '''
        MCC最小类混淆Loss
        '''
        args.t_mcc = 2
        ...
        total_loss = classifier_loss + alignment_loss2 + ditillation_loss + transfer_loss
        # total_loss = classifier_loss + alignment_loss2
