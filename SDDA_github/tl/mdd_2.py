...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        inputs = torch.cat((inputs_source, inputs_target), dim=0)   # torch.Size([64, 1, 3, 1001])
        features, outputs = base_network(inputs)

        p = float(iter_num) / max_iter
        alpha = 2. / (1. + np.exp(-10 * p)) - 1
        reverse_feature = ReverseLayerF.apply(features, alpha)
        domain_output = ad_net(reverse_feature)

        '''
        下面是没改之前的shape
            domain_output_s:  torch.Size([32, 2])
            outputs_source:  torch.Size([32, 2])
            labels_source:  torch.Size([32])
            target_adv:  torch.Size([32])
            target_adv_src:  torch.Size([32])    [非0即1，表示分类结果]
            target_adv_tgt:  torch.Size([0])
        '''
        '''
        下面是现在的shape
            domain_output:  torch.Size([64, 2])
            outputs:  torch.Size([64, 2])
            labels_source:  torch.Size([32])
            target_adv:  torch.Size([64])
            target_adv_src:  torch.Size([32])    [非0即1，表示分类结果]
            target_adv_tgt:  torch.Size([32])
        '''
        classifier_loss = criterion(outputs.narrow(0, 0, labels_source.size(0)), labels_source)
        ...
        classifier_loss_adv_src = criterion(domain_output.narrow(0, 0, labels_source.size(0)), target_adv_src)

        logloss_tgt = torch.log(1 - F.softmax(domain_output.narrow(0, labels_source.size(0), inputs.size(0) - labels_source.size(0)), dim=1))
        classifier_loss_adv_tgt = F.nll_loss(logloss_tgt, target_adv_tgt)

        transfer_loss = args.srcweight * classifier_loss_adv_src + 0*classifier_loss_adv_tgt
