...
def CDANE(input_list, ad_net, random_layer, entropy=None, coeff=None, args=None):
    ...
    if random_layer is not None:     # 为了只执行这个(结果好一点), 条件判断改了一下if random_layer is not None
...
class ClassConfusionLoss(nn.Module):
    ...
    def forward(self, output: tr.Tensor) -> tr.Tensor:
        ...
        mcc_loss = (tr.sum(class_confusion_matrix) - tr.trace(class_confusion_matrix)) / n_class    # loss是标量
        return mcc_loss
